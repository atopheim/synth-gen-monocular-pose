...
from lib.utils.data_utils_homemade_render_fuse import HomemadeImageDB, OcclusionHomemadeImageDB, TruncatedHomemadeImageDB, ValidationDatasetIntake ##
    ...
    for idx, data in enumerate(dataloader):        
        ...
        with torch.no_grad():
            ...
            if (train_cfg['eval_epoch']
                ...
                pose_preds=[]
                for bi in range(b):
                    intri_type='use_intrinsic' if use_camera_intrinsic else 'blender'
...
def train_net():
    ...
    if args.test_model:
        ...
        print('testing ...')
        val_db=ValidationDatasetIntake.getval_dataset()        #image_db.val_real_set
        ...
        val_loader = DataLoader(val_set, batch_sampler=val_batch_sampler, num_workers=0)
        ...
        if train_cfg['use_real_train']:
            ...
            print("DB After real train set: ", len(train_db))  #we have no real images      
