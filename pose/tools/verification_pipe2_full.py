...
train_cfg['model_name'] = '{}_{}'.format('pipe2', train_cfg['model_name'])
...
def read_data(idx):
    ...
    points_3d = np.loadtxt(os.path.join(cfg.HOMEMADE, 'pipe2','pipe2_points_3d.txt')) ##DEEMO DIR
...
def demo(idx):
    ...
    #########NBNBNB#############
    model_dir = os.path.join(cfg.MODEL_DIR, "pipe2_demo") #<-##############################################################3
