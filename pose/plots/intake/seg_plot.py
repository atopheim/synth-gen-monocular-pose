...
with open('/media/volvomlp2/03C796544677EF72/TrainedModels/intake_25-30_(3)/logs/intake_homemade_train.log','r') as csvfile:
    plots = csv.reader(csvfile, delimiter=' ')
    for idx, row in enumerate(plots):
        if (idx+1)%10 == 0:
            x_val.append(int(row[2])) # num epoch
            y_val.append(float(row[6])) # metric
        else:
...
y_val_smooth = sliding_mean(y_val)
